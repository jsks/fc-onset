...
[^robust_censor]: I also run multiple robustness checks in the [appendix][robustness checks] where I alternatively drop episodes with an episode onset year before the coding start for the dataset and/or recurrent episodes within conflicts that also began prior to 1975.
...
#|     sizes showed anticorrelation among the MCMC draws.
...
The first potential issue is that the original definition for a conflict termination as taken from the UCDP Conflict Termination Dataset requires that a conflict drops below a threshold of 25 battle related fatalities for only a single year. This risks including brief pauses in fighting as potential terminations to be coded as frozen conflicts. Although the conceptual definition of frozen conflict that I employ does not include an explicit temporal dimension as a requirement for a conflict to be considered frozen, it could be nonetheless argued that such pauses should not be considered candidates for being classified as frozen since actors may view the conflict as active and ongoing. Therefore, I also construct a stricter measure for frozen conflict whereby a conflict episode has to be followed by more than two years below the UCDP battle related threshold to be considered eligible to being classified as frozen, conditional on the remaining the coding criteria.

The second issue also relates to the underlying population of conflict episodes used to construct the frozen conflict measure. In the original coding for frozen conflict, I included all conflict episodes from the UCDP within the time period from 1975 to 2017, including low-level conflicts with relatively few battle related fatalities. It can be argued that minor, long-running insurgencies should not be considered as frozen conflicts since the underlying issue may no longer be particularly salient and there is a relatively low risk for a major escalation in violence. Such conflicts may experience sporadic, isolated incidences of violence, but would otherwise are minor factors in the domestic political agenda. To this end, I subset the list of conflict episodes to only include civil conflict episodes that have cumulatively experienced at least 1,000 battle related fatalities since onset in the hopes that major conflicts maintain a greater degree of salience post-termination.

Third, due to the truncated nature of the dataset, some of the civil conflict episodes are censored with missing data before 1975. In the main results I included a binary variable if the onset of an episode occurred before the start of the dataset. As an alternative, in the following robustness checks I also drop all censored episodes as well as censored conflicts.

Finally, the primary treatment variables used in the main text are aggregated binary measures for directed support, plus the interaction, at any time during a civil conflict episode. Although this aids in the interpretation of the treatment effect, these variables do not capture the extent to which support is provided during a conflict episode. Thus, I also replace the treatment variables with measures capturing the proportion of conflict-years within an episode that each actor received external support.

In total, this leads to `r list.files("posteriors", "\\d{4}") |> length()` different model configurations for the original vs strict definition of frozen conflict, inclusion and exclusion of low-intensity conflict episodes, inclusion and exclusion of censored episodes and/or conflicts, and finally the proportional aggregation of the treatment variables. @fig-robust
...
rebel <- filter(model_ames, variable == "Ext. Sup. Rebel Only", censored == "with_censored")
ggplot(rebel, aes(pzero, median, colour = treatment)) +
    geom_point(aes(shape = outcome), alpha = 0.8) +
    geom_errorbar(aes(ymin = q5, ymax = q95), alpha = 0.1) +
    geom_vline(xintercept = 0.9, linetype = "dashed", alpha = 0.2) +
    facet_grid(episodes ~ group)

#filter(model_ames, variable == "Ext. Sup. Rebel Only") |>
#ggplot(aes(pzero, median, colour = treatment)) +
